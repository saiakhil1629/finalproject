...
import { FaDownload, FaPlus, FaTrash, FaSave } from "react-icons/fa";
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 h-auto lg:h-[calc(100vh-12rem)] relative">
      ...
      <div className="print:hidden w-full lg:w-1/2 overflow-y-auto pr-4 space-y-6 custom-scrollbar pb-20">
        <div className="flex items-center justify-between sticky top-0 bg-[var(--color-background)]/90 backdrop-blur z-10 py-4 border-b border-white/5">
          <h1 className="text-2xl font-bold text-white tracking-wide">Resume Builder</h1>
          <button 
            onClick={printResume}
            className="flex items-center gap-2 bg-emerald-500 hover:bg-emerald-600 text-white px-4 py-2 rounded-xl text-sm font-medium transition-colors shadow-lg shadow-emerald-500/20"
          >
        ...
        <div className="glass-panel p-6 rounded-2xl border border-white/5">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => addArrayItem("education", { degree: "", institution: "", year: "", gpa: "" })} className="text-emerald-400 hover:text-emerald-300 text-sm flex items-center gap-1">
              <FaPlus /> Add
            </button>
          </div>
          <div className="space-y-4">
            {resumeData.education.map((edu, i) => (
              <div key={i} className="p-4 bg-white/5 rounded-xl border border-white/5 relative group">
                <button onClick={() => removeArrayItem("education", i)} className="absolute top-3 right-3 text-red-400 opacity-0 group-hover:opacity-100 transition-opacity">
                  <FaTrash />
                </button>
        ...
        <div className="glass-panel p-6 rounded-2xl border border-white/5">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => addArrayItem("experience", { role: "", company: "", duration: "", details: "" })} className="text-emerald-400 hover:text-emerald-300 text-sm flex items-center gap-1">
              <FaPlus /> Add
            </button>
          </div>
          <div className="space-y-4">
            {resumeData.experience.map((exp, i) => (
              <div key={i} className="p-4 bg-white/5 rounded-xl border border-white/5 relative group">
                <button onClick={() => removeArrayItem("experience", i)} className="absolute top-3 right-3 text-red-400 opacity-0 group-hover:opacity-100 transition-opacity">
                  <FaTrash />
                </button>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-2">
        ...
        <div className="glass-panel p-6 rounded-2xl border border-white/5">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => addArrayItem("projects", { title: "", tools: "", details: "", link: "" })} className="text-emerald-400 hover:text-emerald-300 text-sm flex items-center gap-1">
              <FaPlus /> Add
            </button>
          </div>
          <div className="space-y-4">
            {resumeData.projects.map((proj, i) => (
              <div key={i} className="p-4 bg-white/5 rounded-xl border border-white/5 relative group">
                <button onClick={() => removeArrayItem("projects", i)} className="absolute top-3 right-3 text-red-400 opacity-0 group-hover:opacity-100 transition-opacity">
                  <FaTrash />
                </button>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-2">
